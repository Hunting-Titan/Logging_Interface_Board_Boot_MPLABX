...
{   
    ...
    /*Set period */ 
    PLIB_TMR_Period16BitSet(TMR_ID_2, 65500);
    /* Setup Interrupt */   
...
{   
    ...
    /* Select prescalar value */
    PLIB_TMR_PrescaleSelect(TMR_ID_7, TMR_PRESCALE_VALUE_16);
    /* Enable 16 bit mode */
    PLIB_TMR_Mode16BitEnable(TMR_ID_7);
    /* Clear counter */ 
    PLIB_TMR_Counter16BitClear(TMR_ID_7);
    /*Set period */ 
    PLIB_TMR_Period16BitSet(TMR_ID_7, 625);
    /* Setup Interrupt */   
    PLIB_INT_VectorPrioritySet(INT_ID_0, INT_VECTOR_T7, INT_PRIORITY_LEVEL3);
...
{   
    ...
    /* Select prescalar value */
    PLIB_TMR_PrescaleSelect(TMR_ID_6, TMR_PRESCALE_VALUE_16);
    /* Enable 16 bit mode */
    PLIB_TMR_Mode16BitEnable(TMR_ID_6);
    /* Clear counter */ 
    PLIB_TMR_Counter16BitClear(TMR_ID_6);
    /*Set period */ 
    PLIB_TMR_Period16BitSet(TMR_ID_6, 65000);
    /* Setup Interrupt */   
...
static DRV_TMR_ALARM_OBJ      DRV_TMR5_DATA;
...
{   
    ...
    /*Set period */ 
    PLIB_TMR_Period16BitSet(TMR_ID_8, 19900);
    /* Initialize the data structure */
    DRV_TMR5_DATA.alarmFunc = NULL;
    DRV_TMR5_DATA.alarmCount = 0;
    DRV_TMR5_DATA.alarmEnabled = false;
    DRV_TMR5_DATA.alarmPeriodic = false;
    /* Setup Interrupt */   
    PLIB_INT_VectorPrioritySet(INT_ID_0, INT_VECTOR_T8, INT_PRIORITY_LEVEL2);
...
)
{
    ...
    if (success)
    {
        ...
        DRV_TMR5_DATA.alarmPeriodic = isPeriodic;
        DRV_TMR5_DATA.alarmContext = context;
        DRV_TMR5_DATA.alarmFunc = callBack;
        return (success);
    }
    else
        return false;
...
{
    if (DRV_TMR5_DATA.alarmFunc != NULL)
    {
        DRV_TMR5_DATA.alarmEnabled = enable;
        PLIB_INT_SourceEnable(INT_ID_0, INT_SOURCE_TIMER_8);
    }
}
...
{
    bool retVal = DRV_TMR5_DATA.alarmEnabled;
        PLIB_INT_SourceDisable(INT_ID_0, INT_SOURCE_TIMER_8);
    DRV_TMR5_DATA.alarmEnabled = false;
...
{
    bool resume = _DRV_TMR5_Suspend();
    DRV_TMR5_DATA.alarmPeriod = value;
    PLIB_TMR_Period16BitSet(TMR_ID_8, (uint16_t)value);
    _DRV_TMR5_Resume(resume);
}
...
{
    uint32_t alarmCountTemp = DRV_TMR5_DATA.alarmCount;
...
{
    if (DRV_TMR5_DATA.alarmEnabled &&
        (DRV_TMR5_DATA.alarmFunc != NULL))
    {	
		DRV_TMR5_DATA.alarmCount++;
        DRV_TMR5_DATA.alarmFunc(
            DRV_TMR5_DATA.alarmContext, 
            DRV_TMR5_DATA.alarmCount);
        if (!DRV_TMR5_DATA.alarmPeriodic)
        {
            DRV_TMR5_AlarmDisable();
        }
    }
}
